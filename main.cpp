...
//	} 
    ...
    for(u16 i=0x2000; i<=0x23BF; i++){
        ...
        if(ppu_mem[i]==0){
            ...
            for(int j=0; j<16; j++){
                if(j < 8) sprite1[j] = ppu_mem[ppu_mem[i] * 0x0010 + j];
                else sprite2[j] = ppu_mem[ppu_mem[i] * 0x0010 + j];
            }
            for(int j=0; j<8; j++){
                for(int k=0; k<8; k++){
                    if((sprite1[j] >> k) == 0 && (sprite2[j] >> k) == 0){
                    ...
                    else if((sprite1[j] >> k) == 0 && (sprite2[j] >> k) == 1){
                    ...
                    else if((sprite1[j] >> k) == 1 && (sprite2[j] >> k) == 0){
                    ...
                    else if((sprite1[j] >> k) == 1 && (sprite2[j] >> k) == 1){
    ...
    cout << "0x3F02, 0 color: " << hex << +colors[ppu_mem[0x3F02]][2] << endl;
